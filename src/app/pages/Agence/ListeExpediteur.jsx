...
import { Button, Table, THead, TBody, Th, Tr, Td, Select } from "components/ui";
...
function TelExpediteurCell({ row }) {
  ...
  if (rawTel) {
    ...
    // Vérifier que c'est un numéro tunisien de 8 chiffres
    if (digits.length === 8) {
      formattedTel = `(+216) ${digits.slice(0, 2)} ${digits.slice(2, 5)} ${digits.slice(5)}`;
    } else if (digits.startsWith('216') && digits.length === 11) {
      // Si le numéro est complet : 216XXXXXXXX
      formattedTel = `(+216) ${digits.slice(3, 5)} ${digits.slice(5, 8)} ${digits.slice(8)}`;
    } else {
      // Si le format ne correspond pas, afficher tel quel (ou ajuster)
      formattedTel = rawTel;
    }
  }
...
function EtatCell({ row }) {
  ...
  // ✅ Extraire l'ID ici, en dehors du useCallback
  const expediteurId = row.original.id;
  ...
  const handleToggle = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/expediteurs/${expediteurId}`, {
        method: 'PATCH',
        headers: {
  ...
  }, [isActive, expediteurId]); // ✅ Dépendances correctes
...
function RowActions({ row, table }) {
  ...
  const openModal = () => {
    console.log("Modal ouvert pour suppression de", row.original.id);
  ...
  const handleDeleteRow = useCallback(async () => {
    ...
    try {
      ...
      // ✅ SUCCÈS : Supprimer immédiatement de la table locale
      ...
      // ✅ ERREUR : Ne pas supprimer, juste afficher l'erreur
      setDeleteError(true);
  ...
  }, [row.original.id, table, confirmDeleteLoading]);
  ...
  return (
    <>
      <div className="flex items-center gap-2">
        <Button
          onClick={() => navigate(`/agence/modifier-expediteur/${row.original.id}`)}
          color="primary"
...
const columns = [
  ...
  columnHelper.accessor((row) => `${row.nom || ''}`.trim(), {
    id: "nomComplet",
    header: "Expéditeur",
    cell: NomCompletCell,
  }),
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Gérez et organisez les expéditeurs de votre agence
            </p>
          </div>
          <Button
            ...
            onClick={() => navigate("/agence/ajouter-expediteur")}
          >
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!agenceId) {
          throw new Error("ID de l'agence non trouvé dans le profil utilisateur");
        }

        // Mettre à jour l'état de l'agence
        setAgence({ id: agenceId, libelle: "Agence par défaut" });

        // Récupérer les expéditeurs
        const expediteursResponse = await fetch(
          `http://localhost:3000/api/expediteurs/agence/${agenceId}`,
          {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json',
        ...
        const enrichedExpediteurs = expediteursData.map((exp) => ({
          ...exp,
          user: exp.user || { nom: '-', prenom: '-', tel: '-' },
          agence: { id: agenceId, libelle: "Agence par défaut" },
        }));
    ...
    fetchData();
  }, [user]); // ✅ Dépendance ajoutée ici

  // Fonction pour supprimer une ligne localement
  const deleteRow = useCallback((id) => {
    setExpediteurs((prev) => prev.filter((exp) => exp.id !== id));
  }, []);
  ...
  return (
    <Page title="Gestion des expéditeurs">
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Toolbar table={table} agence={agence} />
