        ...
        const agencesAvecStats = await Promise.all(
          autresAgences.map(async (agence) => {
            const res = await fetch(
              `http://localhost:3000/api/console-pickup/attente?agence_dest=${agenceDestId}&agence_source=${agence.id}`,
              { headers: { Authorization: `Bearer ${token}` } }
            );
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:gap-6">
          {loading ? (
            ...
            agences.map((agence) => (
              <div
                key={agence.id}
                onClick={() => navigate(`/agence/valider-console-pickup/${agence.id}`)}
