...
import { Table, Button, THead, TBody, Tr, Th, Td } from "components/ui";
import { ArrowPathIcon, PrinterIcon, PencilIcon } from "@heroicons/react/24/outline";
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button style={{backgroundColor:"#FDC633"}}
            onClick={async () => {
              ...
              try {
                setSubmitStatus({ type: 'loading', message: 'Impression en cours...' });

                // 1. Mettre à jour imprime = 1
                const res = await fetch(`http://localhost:3000/api/commandes/update/${row.original.id}`, {
                  method: 'PUT',
                  headers: {
                ...
                // 2. Ouvrir le bordereau
                window.open(`/expediteur/print-bordereau/${row.original.id}`, '_blank');

                // 3. Rafraîchir la liste
                const refetch = await fetch(`http://localhost:3000/api/commandes/imprime/fournisseur/${user.relatedIds.id}/etat/0`, {
                ...
                // 4. Message de succès
                setSubmitStatus({ type: 'success', message: 'Bordereau imprimé avec succès !' });
                setTimeout(() => setSubmitStatus(null), 3000);
              } catch (err) {
                console.error('Erreur impression unique:', err);
                setSubmitStatus({ type: 'error', message: err.message || 'Échec de l’impression.' });
                setTimeout(() => setSubmitStatus(null), 3000);
              }
            }}
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        setError(null);
        const response = await fetch(`http://localhost:3000/api/commandes/imprime/fournisseur/${id_frs}/etat/0`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${authToken}`,
            'Content-Type': 'application/json',
            ...
            <div className="flex items-center justify-between">
              <div className="min-w-0">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  Affichage de vos colis en attente
                </p>
              </div>
            </div>
